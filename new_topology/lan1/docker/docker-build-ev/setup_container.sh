...
if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=controller_ev \
   external_ids:container_iface=eth1 | grep -q "c1_l"; then
        sudo ./my-ovs-docker del-port br_lan1 eth1 controller_ev
fi

if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=controller_ev \
   external_ids:container_iface=eth2 | grep -q "c2_l"; then
        sudo ./my-ovs-docker del-port br1 eth2 controller_ev
fi

# Remember to change containers name if they change
sudo docker stop controller_ev
sudo docker start controller_ev

sudo ./my-ovs-docker add-port br_lan1 eth1 controller_ev c1 20 --ipaddress=10.1.5.100/24
sudo ovs-vsctl -- set port c1_l tag=3
sudo iptables -t nat -A POSTROUTING -o c1_l -j MASQUERADE
sudo ./my-ovs-docker add-port br1 eth2 controller_ev c2 21 --ipaddress=10.1.11.100/24
sudo ovs-vsctl -- set port c2_l tag=11
sudo iptables -t nat -A POSTROUTING -o c2_l -j MASQUERADE

if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=int_host_ev \
   external_ids:container_iface=eth1 | grep -q "h1_l"; then
        sudo ./my-ovs-docker del-port br_lan1 eth1 int_host_ev
fi

sudo docker stop int_host_ev
sudo docker start int_host_ev

sudo ./my-ovs-docker add-port br_lan1 eth1 int_host_ev h1 15 --ipaddress=10.1.3.10/24 --macaddress=08:00:27:b6:d0:66
...
if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=ssh_server \
   external_ids:container_iface=eth1 | grep -q "s1_l"; then
        sudo ./my-ovs-docker del-port br_lan2 eth1 ssh_server
fi

sudo docker stop ssh_server
sudo docker start ssh_server

sudo ./my-ovs-docker add-port br_lan2 eth1 ssh_server s1 22 --ipaddress=10.1.10.11/24 --macaddress=08:00:27:b6:d0:67
...
if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=ext_host \
   external_ids:container_iface=eth1 | grep -q "h2_l"; then
        sudo ./my-ovs-docker del-port br_lan2 eth1 ext_host
fi

sudo docker stop ext_host
sudo docker start ext_host

sudo ./my-ovs-docker add-port br_lan2 eth1 ext_host h2 23 --ipaddress=10.1.10.12/24 --macaddress=08:00:27:b6:d0:68
