...
class NetworkTopology:
    ...
    def __init__(self):
        ...
        self.br1_dpid = 64105189026384
        # Subnets
        # ovs1
        self.subnet1 = Subnet('S1', '10.3.3.0', '255.255.255.0',self.br0_dpid)
        self.subnet2 = Subnet('S2', '10.3.4.0', '255.255.255.0',self.br0_dpid)
        self.subnet3 = Subnet('S3', '10.3.5.0', '255.255.255.0',self.br0_dpid)
        # ovs2
        self.subnet4 = Subnet('S4', '10.3.10.0', '255.255.255.0',self.br1_dpid)
        self.subnet5 = Subnet('S5', '10.3.11.0', '255.255.255.0',self.br1_dpid)

        # Nodes
        self.host = Host('host', '10.3.3.10', '08:00:27:b6:d0:49', 49, '255.255.255.0',self.subnet1)
        self.service = Host('service', '10.3.3.11', '08:00:27:6d:ab:61', 41, '255.255.255.0', self.subnet1)
        self.heralding = Honeypot('heralding', '10.3.3.12', '08:00:27:6c:8a:cc', 42, '255.255.255.0', self.subnet1)

        # Subnet 2
        self.ti_host1 = Host('ti_host1', '10.3.4.10', '08:00:27:b7:0a:51', 43, '255.255.255.0', self.subnet2)
        

        self.cowrie1 = Honeypot('cowrie1', '10.3.4.10', '08:00:27:b7:0a:51', 43, '255.255.255.0', self.subnet2)
        self.heralding1 = Honeypot('heralding1', '10.3.4.10', '08:00:27:b7:0a:51', 43, '255.255.255.0', self.subnet2)
        self.heralding2 = Honeypot('heralding2', '10.3.4.10', '08:00:27:b7:0a:51', 43, '255.255.255.0', self.subnet2)

        self.honeypots_list = [self.cowrie1, self.heralding1, self.heralding2]
        self.hosts_list= [self.ti_host1]
        self.host_redirected = ["10.3.5.1", "10.3.3.1"]

        #self.elk_if1 = Host('ELK_IF1', '10.3.5.10', '08:00:27:7d:b7:b8', 8, '255.255.255.0', self.subnet3)

        

        self.dmz_service = Host('dmz_service1', '10.3.10.14', '08:00:27:6d:ec:52', 36, '255.255.255.0', self.subnet4)

        self.nodes = [self.host, self.service,self.heralding,self.dmz_service]
        
        
        # Gateways
        # ovs1
        self.gw1 = Gateway('gw1', '10.3.3.1', '8a:ae:02:40:3f:96', 50, '255.255.255.0',self.subnet1)
        self.gw2 = Gateway('gw2', '10.3.4.1', 'ea:6a:20:a0:36:15', 51, '255.255.255.0',self.subnet2)
        self.gw3 = Gateway('gw3', '10.3.5.1', 'ea:6a:20:a0:36:17', 52, '255.255.255.0', self.subnet3)
        # ovs2
        self.gw10 = Gateway('gw10', '10.2.10.1', '8a:ae:02:40:3f:93', 56, '255.255.255.0', self.subnet4)
        self.gw11 = Gateway('gw11', '10.1.11.1', 'ea:6a:20:a0:36:11', 57, '255.255.255.0', self.subnet5)
