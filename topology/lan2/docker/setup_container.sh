...
if sudo ovs-vsctl --data=bare --no-heading --columns=name find interface external_ids:container_id=controller \
   external_ids:container_iface=eth2 | grep -q "c2_l"; then
        sudo ./my-ovs-docker del-port br_lan2 eth2 controller
fi
...
sudo ./my-ovs-docker add-port br_lan2 eth1 ssh_server s1 22 --ipaddress=10.2.3.11/24 --macaddress=08:00:27:b6:d0:67
...
sudo ./my-ovs-docker add-port br_lan2 eth1 ext_host h2 23 --ipaddress=10.2.3.12/24 --macaddress=08:00:27:b6:d0:68
